function TestVigFourLevels(Subj)
...
testing_screen=[0 0 177 100]*6; % or [] for the whole screen

Trials_per_block=32;
% Timings
refresh_rate=60; % Time is calcaulted relative to refresh rate e.g.,
% when refresh rate is set to 60 Hz, 30 means 0.5 second
trial_length=200;
distance_between_dots_limit=(trial_length./10);
distance_between_events_limit=randsample([fix(trial_length./6) fix(trial_length./5)],1);
refractory_time=30;
post_defl_refractory_time=15;
fading_time=20;
...
non_target_time_gap_constant=0.2494;

percentage_target_cond1=0.5;
percentage_target_cond2=0.24;
percentage_target_cond3=0.12;
percentage_target_cond4=0.06;

first_dot_colour=[255 0 0];
second_dot_colour=[0 0 255];
distance_of_consideration=600;

Num_of_conditions=4;
Blocks_per_condition=1;
Sets_of_subjects=1; % how many groups of subjects do you want to repeat all the conditions?
...
for subj=1:size(conds_perm,1)
    for cond=1:Num_of_conditions        
        indices_this_block=(cond-1)*Blocks_per_condition+1:cond*Blocks_per_condition;
        Block_condition(subj,indices_this_block)=repmat(conds_perm(subj,cond),[1 Blocks_per_condition]);
        indices_to_make_zero=randsample(indices_this_block,round(size(indices_this_block,2)./2));
...
try
    ...
    if Eye_tracking
        ...
        [wpoint, wRect]=Screen('OpenWindow', screenNumber, 0,testing_screen,32,2);
    ...
    for Block_Num=1:Blocks_per_condition
        if Block_condition(Subj,Block_Num)==1
            percentage_target=percentage_target_cond1;
            Task='Condition1';
        elseif Block_condition(Subj,Block_Num)==2
            Task='Condition2';
            percentage_target=percentage_target_cond2;
        elseif Block_condition(Subj,Block_Num)==3
            Task='Condition3';
            percentage_target=percentage_target_cond3;
        elseif Block_condition(Subj,Block_Num)==4
            Task='Condition4';
            percentage_target=percentage_target_cond4;        
        end
        
        if Block_Num==1
            background_colour=0;
            [wpoint, wRect]=Screen('OpenWindow',screenNumber,background_colour,testing_screen);
            if SaveMovie==1
        ...
        if Eye_tracking
            ...
            Eyelink('command', 'record_status_message "BLOCK %d/%d, Task %s, Target %s"', Block_Num, Blocks_per_condition, Task, target_color);
...
%         save(['Subj_',num2str(Subj),'_Blk_',num2str(Block_Num),'_Cond_',num2str(Block_condition(Subj,Block_Num)),...
%             '_test_FourLevels.mat'],'manually_deflected_top','top_events','top_events2','top_targets','top_targets2',...
