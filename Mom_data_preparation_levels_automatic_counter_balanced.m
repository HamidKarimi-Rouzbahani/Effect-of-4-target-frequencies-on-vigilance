...
subjects=[1 2] ; % subjects you want the include in analysis
Testing_blocks=[1:40]; % blocks you want to include in analysis
    ...
    correct_reaction_times_att=0;
        for i=3:size(dirs,1)
            if strcmp(dirs(i).name(end-3:end),'.mat')
                if strcmp(dirs(i).name(1:10+length(num2str(Subj))+length(num2str(blk))+1),['Subj_',num2str(Subj),'_Blk_',num2str(blk),'_'])  %strcmp(dirs(i).name(9+length(num2str(subj)):9+length(num2str(subj))+length(num2str(blk))),num2str(blk)) &&
                    load(dirs(i).name);
...
for Subj=subjects
    ...
    filename = ['MoM_data_Levels_automatic_counter_balancing.xlsx']; % Change the name to anything you prefer
...
RT=1; % 1 for reaction time and 0 for hit rate
if RT==0
    dataA1=(1-Data{1,2}(:,subjects))*100;
    dataB1=(1-Data{2,2}(:,subjects))*100;
    dataC1=(1-Data{3,2}(:,subjects))*100;
    dataD1=(1-Data{4,2}(:,subjects))*100;
else
...
legend([Shad1,Shad2,Shad3,Shad4],{'Target Freq. 1','Target Freq. 2', 'Target Freq. 3' 'Target Freq. 4'},'location','northwest','edgecolor','none')